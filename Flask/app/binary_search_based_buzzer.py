...
def get_tfidf_for_words(text, tfidf):
    ...
    foodict = {
        k: v for k, v in dict(tfidf_scores).items() if k in break_into_words(text[0])
    }
    # print(foodict, text)
            ...
            if guesses[0][1] < threshold_buzz:
                for i in range(len(guesses)):
                    if guesses[i][0] == ans.replace(" ", "_"):
                        if ans != "":
                            return (
                                "Buzzer does not cross the threshold: Ans is at rank "
                                + str(i)
                                + " with confidence "
                                + str(round(guesses[i][1], 2)),
                                "",
                                False,
                                "",
                                -1,
        ...
        if guesses[0][1] > threshold_buzz:
            ...
            for i in range(len(guesses)):
                if guesses[i][0] == ans.replace(" ", "_"):
                    if ans != "":
                        if i == 0:
                            if guesses[i][1] >= threshold_buzz:
                                set_flag = 2
                                break
                            else:
                                return (
                                    "Buzzer does not cross the threshold, but it is the top guess with confidence "
                                    + str(round(guesses[i][1], 2)),
                                    "",
                                    False,
                                    "",
...
def get_actual_guess_with_index(question, max=12):
    ...
    foodict = {
        k: v
        for k, v in dict(tfidf_scores).items()
        if k not in break_into_words(question[0])
    }
    # print(foodict)
    sort_orders = sorted(foodict.items(), key=lambda x: x[1], reverse=True)[:4]
...
def get_importance_of_each_sentence(question):
    ...
    """
    actual_answer, index_of_answer, highlight_words = get_actual_guess_with_index(
        question=[question]
    )
    ...
    for i in range(len(array_of_importances)):
        ...
        most_important.append(
            {
                "sentence": a[0] + " ... " + a[-1],
                "importance": round(array_of_importances[i], 3),
            }
        )
    return (
        most_important,
        temp_sentence_array[highest_confidence_sentence],
        highest_confidence_sentence + 1,
        highlight_words,
    )
...
def insert_into_db(
    ...
    ans,
    buzzer_string,
    buzzer_sentence_number,
    buzzer_word_number,
    most_important_sentence_number,
    ...
    if q_id not in buzzer:
        buzzer[q_id] = []
        state_buzzer[q_id] = {"ans": False, "pos": -1}
    is_relevant = False
    string_new = ""
        ...
        if if_ans_found == 1:
            ...
            if previous_pos != -1:
                if buzzer_word_number < previous_pos:
                    string_new = (
                        "Buzzer buzzed earlier than previous position by "
                        + str(previous_pos - buzzer_word_number)
                        + "postions"
                    )
                else:
                    string_new = (
                        "Buzzer buzzed later than previous position by "
                        + str(buzzer_word_number - previous_pos)
                        + "postions"
                    )
            else:
...
def buzz_full_question():
        ...
        return jsonify(
            {
                ...
                "flag": False,
                "top_guess": "",
                "importance": [{"sentence": "-", "importance": -1}],
            }
        )
    start = time.time()
    (
    ...
    buzz_word = []
    print(
        "----TIME (s) : /binary_search_based_buzzer/buzz_full_question---", end - start
    )
    ...
    if flag:
        (
            ...
            sentence_number,
            highlight_words,
        ) = get_importance_of_each_sentence(buzzer_string)
        ...
        insert_into_db(
            ...
            ans,
            buzzer_string,
            len(break_into_sentences(buzzer_string)),
            buzzer_word_number,
            sentence_number,
            sentence_string,
            top_guess,
    ...
    # prev_highlight[q_id] = highlight_words + [x.capitalize() for x in highlight_words] + [x.lower() for x in highlight_words]
    print(
        "----TIME (s) : /binary_search_based_buzzer/get_importance_sentence---",
        end - start,
    )
    return jsonify(
        {
            ...
            "flag": flag,
            "top_guess": top_guess,
            "importance": importance_sentence,
            "buzzer_last_word": buzzer_last_word,
            "highlight_words": highlight_words,
            "remove_highlight": temp_highlight,
        }
    )
